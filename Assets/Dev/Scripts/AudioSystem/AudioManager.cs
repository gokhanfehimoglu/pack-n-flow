...
namespace PackNFlow.AudioSystem
{
	public class AudioManager : Singleton<AudioManager>
	{
		public bool IsSoundMuted
		{
			get
			{
				return PlayerPrefs.GetInt(GameSaveKeys.IsSoundMuted, 0).Equals(1);
			}
			set
			{
				PlayerPrefs.SetInt(GameSaveKeys.IsSoundMuted, value ? 1 : 0);
				ToggleAudioTrack(0, value);
				ToggleAudioTrack(1, value);
			}
		}
		public bool IsMusicMuted
		{
			get
			{
				return PlayerPrefs.GetInt(GameSaveKeys.IsMusicMuted, 0).Equals(1);
			}
			set
			{
				PlayerPrefs.SetInt(GameSaveKeys.IsMusicMuted, value ? 1 : 0);
			}
		}
		...
		private void Init()
		{
			...
			ToggleAudioTrack(1, IsSoundMuted);
		}
		...
		public void ToggleAudioTrack(int index, bool mute)
		{
			...
			if (mute)
				mixer.audioMixer.SetFloat(VOLUME_PARAM, -500f);
			else
				mixer.audioMixer.SetFloat(VOLUME_PARAM, 2.6f);
		}
		...
		private IEnumerator RunAudioJob(AudioTrack track, AudioJob job, Vector3 audioPosition = default)
		{
			...
			if (track.CanPlayOver)
			{
				...
				DOVirtual.DelayedCall(job.Params.Length.Equals(0) ? source.clip.length : job.Params.Length, () => track.AudioSourcePool.Release(source));
			}
			else
			{
